...
public class AllyLocationBroadcaster {
	...
	public void tickAll(MinecraftServer server) {
		...
		for (ServerPlayer receiver : server.getPlayerList().getPlayers()) {
			...
			PacketDistributor.sendToPlayer(receiver, new AllyLocationsS2C(buildEntriesFor(receiver, server, wantsParty, wantsAllies)));
		}
	}
	...
	private List<AllyLocationsS2C.Entry> buildEntriesFor(
			ServerPlayer receiver, MinecraftServer server, boolean wantsParty, boolean wantsAllies) {
		...
		for (UUID candidateUuid : candidates) {
			ServerPlayer candidate = server.getPlayerList().getPlayer(candidateUuid);
			if (candidate == null) {
				continue;
			}
			if (!candidate.level().dimension().equals(receiver.level().dimension())) {
				continue;
			}
