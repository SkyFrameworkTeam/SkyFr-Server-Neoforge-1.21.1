...
public final class EndSpawnPlatformGenerator {
	...
	public static void generate(ServerLevel world, BlockPos spawnPos) {
		for (int dx = -RADIUS; dx <= RADIUS; dx++) {
			for (int dz = -RADIUS; dz <= RADIUS; dz++) {
				BlockPos ground = spawnPos.offset(dx, -1, dz);
				world.setBlockAndUpdate(ground, Blocks.END_STONE.defaultBlockState());
				world.setBlockAndUpdate(ground.above(), Blocks.AIR.defaultBlockState());
				world.setBlockAndUpdate(ground.above(2), Blocks.AIR.defaultBlockState());
			}
		}
	}
}
