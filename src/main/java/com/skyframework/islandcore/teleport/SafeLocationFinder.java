...
public final class SafeLocationFinder {
	...
	public static Optional<BlockPos> findNearestSafe(ServerLevel world, BlockPos target, int maxSearchRadius) {
	...
	public static Optional<BlockPos> findNearestSafe(ServerLevel world, BlockPos target, int maxSearchRadius, IslandBounds bounds) {
	...
	private static Optional<BlockPos> searchRing(ServerLevel world, BlockPos target, int y, int radius, IslandBounds boundsFilter) {
	...
	public static Optional<BlockPos> findNearestSafeHeightAware(ServerLevel world, BlockPos target, int maxRadius) {
		if (SafeLandingChecker.isSafe(world, target)) {
			return Optional.of(target);
		}
	...
	public static int resolveTopY(ServerLevel world, int x, int z) {
		if (!world.dimensionType().hasCeiling()) {
			return world.getHeight(Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, x, z);
		}

		Predicate<BlockState> blocking = Heightmap.Types.MOTION_BLOCKING_NO_LEAVES.isOpaque();
		int scanBottom = world.getMinBuildHeight();
		int scanTop = scanBottom + world.dimensionType().logicalHeight() - 1;
		BlockPos.MutableBlockPos pos = new BlockPos.MutableBlockPos(x, scanTop, z);
