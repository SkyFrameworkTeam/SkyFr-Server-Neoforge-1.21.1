...
public final class ExceptionResolver {
	...
	public static Optional<ExceptionGroup> resolve(Island island, UUID playerUuid, ResourceLocation objectId, ExceptionGroupCategory category) {
	...
	private static boolean matchesAny(ExceptionGroup group, ResourceLocation objectId, ExceptionGroupCategory category) {
	...
	private static boolean matchesTag(String tagPath, ResourceLocation objectId, ExceptionGroupCategory category) {
		ResourceLocation tagId = ResourceLocation.parse(tagPath);

		if (category == ExceptionGroupCategory.BLOCK) {
			Block block = BuiltInRegistries.BLOCK.get(objectId);
			TagKey<Block> tagKey = TagKey.create(Registries.BLOCK, tagId);
			return block.builtInRegistryHolder().is(tagKey);
		}

		EntityType<?> entityType = BuiltInRegistries.ENTITY_TYPE.get(objectId);
